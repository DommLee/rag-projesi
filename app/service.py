...
class BISTAgentService:
    ...
    def _ingest_with_registry(
        ...
        self.metrics["ingest_docs_seen"] += int(stats.get("total_docs_seen", 0))
        self.metrics["ingest_docs_skipped"] += int(stats.get("skipped", 0))
        self.last_ingest_stats = stats
        return inserted

    def ingest_kap(self, request: IngestRequest) -> int:
        return self._ingest_with_registry(request, self.kap_ingestor.collect, "kap_ingest_chunks")

    def ingest_news(self, request: IngestRequest) -> int:
        return self._ingest_with_registry(request, self.news_ingestor.collect, "news_ingest_chunks")

    def ingest_report(self, request: IngestRequest) -> int:
        return self._ingest_with_registry(request, self.report_ingestor.collect, "report_ingest_chunks")
