...
configure_logging()
app = FastAPI(title="BIST Agentic RAG", version="1.1.0")
...
<body>
  <div class="wrap">
    ...
    <div class="card">
      <h2>Evaluation</h2>
      <label>Mode</label>
      <select id="evalMode"><option>hybrid</option><option>mock</option><option>real</option></select>
...
def query_insight(request: QueryRequest) -> dict:
    try:
...
def diagnostics(ticker: str) -> dict:
...
def create_ingest_job(source_type: str, request: IngestRequest) -> JobRecord:
    source_type = source_type.lower()
